...
class PostTest extends DuskTestCase
{
    ...
    public function testCantPostWithoutImage()
    {
        $this->browse(function (Browser $browser) {
            ...
            $browser->visit('http://127.0.0.1/soen341/public/posts/create')
					->assertUrlIs('test')
    ...
    public function testPost()
    {
        ...
        $this->browse(function (Browser $browser) use ($user) {
            ...
            $browser->visit('http://127.0.0.1/soen341/public/login')
					->type('email',$user->email)
					->type('password','password')
					->press('Login')
					->visit('http://127.0.0.1/soen341/public/posts/create')
                    ->type('body','Test body')
                    ->type('title','Test Title')
                    ->attach('image','./tests/testimages/test.png')
					->press('Submit')
